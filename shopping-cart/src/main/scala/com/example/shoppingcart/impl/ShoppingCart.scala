...
object ShoppingCart {
  ...
  final case class RemoveAll(id: String, replyTo: ActorRef[Confirmation]) extends Command
  ...
  final case class AllItemsRemoved(id: String) extends Event
  ...
  implicit val allItemsRemovedFormat: Format[AllItemsRemoved]           = Json.format
...
final case class ShoppingCart(items: Map[String,Int], checkedOutTime: Option[Instant] = None) {
  ...
  def applyCommand(cmd: Command): ReplyEffect[Event, ShoppingCart] =
    if (isOpen) {
      cmd match {
        ...
        case RemoveAll(id, replyTo)                        => onRemoveAll(id, replyTo)
        ...
        case GetItemCount(replyTo)                         => onGetItemCount(replyTo)
      }
    } else {
      cmd match {
        case Get(replyTo)                      => onGet(replyTo)
        ...
        case RemoveAll(id, replyTo)            => reply(replyTo)(Rejected("Cannot remove items from a checked-out cart"))
  ...
  private def onRemoveAll(id: String,replyTo: ActorRef[Confirmation]): ReplyEffect[Event, ShoppingCart] = {
    if (items.nonEmpty)
      Effect
        .persist(AllItemsRemoved(id))
        .thenReply(replyTo)(updatedCart => Accepted(toSummary(updatedCart)))
    else
      Effect.reply(replyTo)(Rejected("Cannot clear an empty cart")) // removing an item is idempotent
  }
  ...
  private def onGetItemCount(replyTo: ActorRef[Summary]): ReplyEffect[Event, ShoppingCart] = {
    reply(replyTo)(toSummaryCount(this))
...
object ShoppingCartSerializerRegistry extends JsonSerializerRegistry {
  ...
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    ...
    JsonSerializer[AllItemsRemoved],
